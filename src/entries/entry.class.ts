...
export class Entry {
   ...
   */
  public static getPicks(entryId: number): Promise<EntryPick[]> {

    return new Promise((resolve, reject) => {

      Promise.all([Utils.getAllPlayers(), Entry.getEvents(entryId)]).then((result) => {
        ...
        async.each(gameweeks, (gameweek, nextGameweek) => {
          ...
          Promise.all([Utils.getPlayersByEvent(event),
          Entry.getPicksByEvent(entryId, event)]).then((result1) => {
   ...
   * @param entryId
   */
  public static getStats(entryId: number): Promise<EntryStats> {

    return new Promise((resolve, reject) => {

      Promise.all([
        Entry.getDetails(entryId),
        Entry.getEvents(entryId)]).then((result) => {
   ...
   * @param event The event number
   */
  public static async getPicksByEvent(entryId: number, event: number): Promise<Pick[]> {
